package com.kh.khdata.dao;

import java.util.List;
import java.util.Set;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Repository;

import com.kh.khdata.dto.MemberDto;
import com.kh.khdata.mapper.MemberMapper;

@Repository
public class MemberDao {
	
	@Autowired
	private JdbcTemplate jdbcTemplate;
	
	@Autowired
	private MemberMapper memberMapper;
	
	public void insert(MemberDto memberDto) {
		String sql = "insert into member "
				+ "( "
				+ "member_id, member_pw, member_nickname, "
				+ "member_birth, member_contact, member_email, "
				+ "member_post, member_address1, member_address2, "
				+ "member_join, member_change "
				+ ") "
				+ "values "
				+ "( "
				+ "?, ?, ?, ?, ?, ?, ?, ?, ?, systimestamp, systimestamp "
				+ ")";
		
		Object[] params = {
				memberDto.getMemberId(),
				memberDto.getMemberPw(),
				memberDto.getMemberNickname(),
				memberDto.getMemberBirth(),
				memberDto.getMemberContact(),
				memberDto.getMemberEmail(),
				memberDto.getMemberPost(),
				memberDto.getMemberAddress1(),
				memberDto.getMemberAddress2()
		};
		
		jdbcTemplate.update(sql, params);
	}
	
	public List<MemberDto> selectList() {
		String sql = "select * from member "
				+ "where member_level != '관리자' "
				+ "order by member_id asc";
		
		return jdbcTemplate.query(sql, memberMapper);
	}
	
	public List<MemberDto> selectList(String column, String keyword) {
		Set<String> allowList = Set.of("member_id", "member_nickname","member_email","member_contact", "member_level");
		if(allowList.contains(column) == false) List.of();
		
		String sql = "select * from member "
				+ "where instr(#1, ?) > 0 and where member_level != '관리자'"
				+ "order by #1 asc, member_id asc";
		sql = sql.replace("#1", column);
		
		Object[] params = { keyword };
		
		return jdbcTemplate.query(sql, memberMapper, params);
	}
	
	public MemberDto selectOne(String memberId) {
		String sql = "select * from member where member_id = ?";
		
		Object[] params = { memberId };
		
		List<MemberDto> memberList = jdbcTemplate.query(sql, memberMapper, params);
		
		return memberList.isEmpty() ? null : memberList.get(0);
	}
	
	public MemberDto selectOneBymemberIdAndmemberPw(String memberId, String memberPw) {
	    String sql = "select * from member where member_id = ? and member_pw = ?";
	    Object[] params = { memberId, memberPw };
	    List<MemberDto> memberList = jdbcTemplate.query(sql, memberMapper, params);
	    return memberList.isEmpty() ? null : memberList.get(0);
	}
	
	public MemberDto selectOneByMemberNickname(String memberNickname) {
		String sql = "select * from member where member_nickname = ?";
		
		Object[] params = { memberNickname };
		
		List<MemberDto> memberList = jdbcTemplate.query(sql, memberMapper, params);
		
		return memberList.isEmpty() ? null : memberList.get(0);
	}
	
	//정보를 변경하는 경우는 크게 두가지가 있다
	//1. 회원이 자신의 정보를 변경하는 경우
	//2. 관리자가 회원의 정보를 변경하는 경우
//	public boolean login(String memberId) {
//		String sql = "update member set member_login = systimestamp where member_id = ?";
//	    Object[] params = { memberId };
//	    return jdbcTemplate.update(sql, params) > 0;
//	}
	
	public boolean updateMember(MemberDto memberDto) {
		String sql = "update member "
				+ "set "
				+ "member_nickname = ?, member_birth = ?, "
				+ "member_contact = ?, member_email = ? , "
				+ "member_post = ?, member_address1 = ?, "
				+ "member_address2 =? "
				+ "where member_id = ?";
		
		Object[] params = {
				memberDto.getMemberNickname(),
				memberDto.getMemberBirth(),
				memberDto.getMemberContact(),
				memberDto.getMemberEmail(),
				memberDto.getMemberPost(),
				memberDto.getMemberAddress1(),
				memberDto.getMemberAddress2(),
				memberDto.getMemberId()	
		};
		
		return jdbcTemplate.update(sql, params) > 0;
	}
	
	public boolean isUpdatePassword(String memberId) {
		String sql = "select * from member where member_id = ? "
				+ "and member_change < systimestamp - interval '30' day";
		
		Object[] params = { memberId };
		
		return jdbcTemplate.update(sql, memberMapper, params) > 0;
	}
	
	public boolean updateMemberLogin(String memberId) {
		String sql = "update member set member_login=systimestamp where member_id=?";
		Object[] params = { memberId };
		return jdbcTemplate.update(sql, params) > 0;
	}
	
	public boolean updateMemberPw(MemberDto findDto) {
		return updateMemberPw(findDto.getMemberId(), findDto.getMemberPw());
		
	}
	public boolean updateMemberPw(String loginId, String changePw) {
		String sql = "update member set "
				+ "member_pw = ?, "
				+ "member_change = systimestamp "
				+ "where member_id = ? ";
		Object[] params = {changePw, loginId};
		
		return jdbcTemplate.update(sql, params) > 0;
	}
	

	public boolean updateMemberByAdmin(MemberDto memberDto) {
		String sql = "update member "
				+ "set "
				+ "member_nickname = ?, member_birth = ?, "
				+ "member_contact = ?, member_email = ? , "
				+ "member_post = ?, member_address1 = ?, "
				+ "member_address2 =?, member_level = ?, "
				+ "member_point = ? "
				+ "where member_id = ?";
		
		Object[] params = {
				memberDto.getMemberNickname(),
				memberDto.getMemberBirth(),
				memberDto.getMemberContact(),
				memberDto.getMemberEmail(),
				memberDto.getMemberPost(),
				memberDto.getMemberAddress1(),
				memberDto.getMemberAddress2(),
				memberDto.getMemberLevel(),
				memberDto.getMemberPoint(),
				memberDto.getMemberId()
		};
		
		return jdbcTemplate.update(sql, params) > 0;
	}
	
	public boolean delete(String memberId) {
		String sql = "delete member where member_id = ?";
		
		Object[] params = { memberId };
		
		return jdbcTemplate.update(sql, params) > 0;
	}

//	public int count(PageVO pageVO) {
//		if (pageVO.isList()) {
//			return 0;
//		} else {
//			String sql = "select count(*) from member " + "where instr(#1, ?) > 0 and member_level != '관리자'";
//			sql = sql.replace("#1", pageVO.getColumn());
//
//			Object[] params = { pageVO.getKeyword() };
//
//			return jdbcTemplate.queryForObject(sql, int.class, params);
//		}
//	}
//
//	public List<MemberDto> selectListWithPaging(PageVO pageVO) {
//		if (pageVO.isList()) {
//			return null;// 목록은 현재 보여주지 않고 있다
//		} else { // 검색이라면
//			String sql = "select * from ( " + "select rownum rn, TMP.* from ( " + "select * from member "
//					+ "where instr(#1, ?) > 0 and member_level != '관리자' " + "order by #1 asc, member_id asc " + ")TMP "
//					+ ") where rn between ? and ?";
//
//			sql = sql.replace("#1", pageVO.getColumn());
//
//			Object[] params = { pageVO.getKeyword(), pageVO.getBegin(), pageVO.getEnd() };
//
//			return jdbcTemplate.query(sql, memberMapper, params);
//		}
//	}
	
	//회원 프로필 가능
	public void connect(String memberId, int attachmentNo) {
		String sql = "insert into member_profile(member_id, attachment_no) "
				+ "values (?, ?)";
		
		Object[] params = { memberId, attachmentNo };
		
		jdbcTemplate.update(sql, params);
	}
	
	public int findAttachment(String memberId) {
		String sql = "select attachment_no from member_profile where member_id = ?";
		
		Object[] params = { memberId };
		
		return jdbcTemplate.queryForObject(sql, int.class, params);
	}
}



